...
if rol_usuario == 'alumno':
    ...
    while seguir_calificando == 's':
        ...
        # INPUT
        # Estos dos "while" YA evitan el KeyError: no dejan avanzar hasta
        # que el alumno / la materia realmente existan.
        ...
        # PROCESS
        # NUEVO: manejo de errores para la calificación (antes se aceptaba
        # cualquier texto, incluso algo no numérico o fuera de rango).
        calificacion_valida = False
        while not calificacion_valida:
            entrada_calificacion = input('Nueva calificación: ')
            try:
                calificacion_nueva = float(entrada_calificacion)
                if 0 <= calificacion_nueva <= 10:
